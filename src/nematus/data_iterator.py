...
class TextIterator:
    ...
    def reset(self):
        ...
        epoch_num += 1
        if self.shuffle:
            shuffle.main([self.source.name.replace('.shuf',''), self.target.name.replace('.shuf','')], epoch_num)
        ...
        try:
            ...
            while True:
                ...
                for w in ss:
                    ...
                    w = w if w in self.embeddings else "<UNK>"
                    tmp.append(w)
                ss = tmp
...
class TextIteratorEmbedding:
    ...
    def next(self):
        ...
        vlens = [len(s) for s in source]
        max_timesteps = max(vlens)
        new_source = numpy.zeros((max_timesteps, len(source), self.embeddings.shape[1]))
        print new_source.shape
        print len(source), len(source[0])
        for i, s in enumerate(source):
            s = numpy.array(s)
            new_source[:len(s), i, :] = s.reshape((s.shape[0], s.shape[2]))
