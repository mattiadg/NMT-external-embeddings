...
def param_init_residual(options, params, prefix='ff', nin=None,
                       ortho=True):
    if nin is None:
        nin = options['dim_proj']
    params[pp(prefix, 'W')] = norm_weight(nin, nin, scale=0.01, ortho=ortho)
    params[pp(prefix, 'b1')] = numpy.zeros((nin,)).astype('float32')
    params[pp(prefix, 'b2')] = numpy.zeros((nin,)).astype('float32')

    return params

def residual_layer(tparams, state_below, options, prefix='rconv',
            activ='lambda x: tensor.tanh(x)', **kwargs):
    hidden = eval(activ)(
        tensor.dot(state_below, tparams[pp(prefix, 'W')]) +
        tparams[pp(prefix, 'b1')])
    return eval(activ)(
        hidden + state_below + tparams[pp(prefix, 'b2')]
    )
...
def gru_layer(tparams, state_below, options, prefix='gru', mask=None,
    ...
    def _step_slice(m_, x_, xx_, h_, U, Ux, rec_dropout):
        ...
        h = u * h_ + (1. - u) * h
        h = tensor.cast(m_[:, None] * h + (1. - m_)[:, None] * h_, theano.config.floatX)
