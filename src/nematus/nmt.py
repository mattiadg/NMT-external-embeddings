...
# batch preparation
def prepare_data(seqs_x, seqs_y, embeddings, maxlen=None, n_words_src=30000,
                 n_words=30000, unk_token='<UNK>'):
    ...
    n_factors = embeddings.shape[1]
    maxlen_x = numpy.max(lengths_x) + 1
    maxlen_y = numpy.max(lengths_y) + 1

    x = numpy.zeros((maxlen_x, n_samples, n_factors)).astype('float32')
    y = numpy.zeros((maxlen_y, n_samples)).astype('int64')
    x_mask = numpy.zeros((maxlen_x, n_samples)).astype('float32')
    y_mask = numpy.zeros((maxlen_y, n_samples)).astype('float32')
    for idx, [s_x, s_y] in enumerate(zip(seqs_x, seqs_y)):
        for j, word in enumerate(s_x):
            x[j, idx, :] = embeddings.get(word)
        x[j+1, idx, :] = embeddings.get('</S>')
...
# initialize all parameters
def init_params_embeddings(options):
    params = OrderedDict()
    ...
    params = get_layer_param('ff')(options, params, prefix='ff_map',
                                   nin=options['dim_external'], nout=options['dim_word'])
    params = get_layer_param('ff')(options, params, prefix='ff_mapr',
                                   nin=options['dim_external'], nout=options['dim_word'])
    # encoder: bidirectional RNN
    params = get_layer_param(options['encoder'])(options, params,
                                              prefix='encoder',
...
# bidirectional RNN encoder: take input x (embeddings), and produce sequence of context vectors (ctx)
# only for decoding.
# The embeddings must have size [n_timesteps, n_samples, options['dim_word']]
def build_encoder_embeddings(tparams, options, trng, use_noise, x_mask=None, sampling=False):
    x = tensor.tensor3('x', dtype=tensor.config.floatX)
    x.tag.test_value = (numpy.random.rand(1, 5, 10) * 100).astype(tensor.config.floatX)
    ...
    if options['use_dropout']:
        retain_probability_emb = 1 - options['dropout_embedding']
        retain_probability_map = 1 - options['dropout_map']
        retain_probability_hidden = 1 - options['dropout_hidden']
        #retain_probability_source = 1 - options['dropout_source']
        if sampling:
            if options['model_version'] < 0.1:
                rec_dropout = theano.shared(numpy.array([retain_probability_hidden] * 2, dtype='float32'))
                rec_dropout_r = theano.shared(numpy.array([retain_probability_hidden] * 2, dtype='float32'))
                emb_dropout = theano.shared(numpy.array([retain_probability_emb] * options['dim_external'], dtype='float32'))
                emb_dropout_r = theano.shared(numpy.array([retain_probability_emb] * options['dim_external'], dtype='float32'))
                map_dropout = theano.shared(numpy.array([retain_probability_map] * options['dim_word'], dtype='float32'))
                map_dropout_r = theano.shared(numpy.array([retain_probability_map] * options['dim_word'], dtype='float32'))
                #source_dropout = theano.shared(numpy.float32(retain_probability_source))
            else:
                rec_dropout = theano.shared(numpy.array([1.] * 2, dtype='float32'))
                rec_dropout_r = theano.shared(numpy.array([1.] * 2, dtype='float32'))
                emb_dropout = theano.shared(numpy.array([1.] * options['dim_external'], dtype='float32'))
                emb_dropout_r = theano.shared(numpy.array([1.] * options['dim_external'], dtype='float32'))
                map_dropout = theano.shared(numpy.array([1.] * options['dim_word'], dtype='float32'))
                map_dropout_r = theano.shared(numpy.array([1.] * options['dim_word'], dtype='float32'))
                #source_dropout = theano.shared(numpy.float32(1.))
        else:
            if options['model_version'] < 0.1:
                scaled = False
            ...
            rec_dropout_r = shared_dropout_layer((2, n_samples, options['dim']), use_noise, trng,
                                                 retain_probability_hidden, scaled)
            emb_dropout = shared_dropout_layer((n_timesteps, n_samples, options['dim_external']), use_noise, trng,
                                               retain_probability_emb, scaled)
            emb_dropout_r = shared_dropout_layer((n_timesteps, n_samples, options['dim_external']), use_noise, trng,
                                                 retain_probability_emb, scaled)
            map_dropout = shared_dropout_layer((n_timesteps, n_samples, options['dim_word']), use_noise, trng,
                                               retain_probability_map, scaled)
            map_dropout_r = shared_dropout_layer((n_timesteps, n_samples, options['dim_word']), use_noise, trng,
                                                 retain_probability_map, scaled)
        ...
        rec_dropout = theano.shared(numpy.array([1.] * 2, dtype='float32'))
        rec_dropout_r = theano.shared(numpy.array([1.] * 2, dtype='float32'))
        emb_dropout = theano.shared(numpy.array([1.] * n_timesteps, dtype='float32'))
        emb_dropout_r = theano.shared(numpy.array([1.] * n_timesteps, dtype='float32'))
        map_dropout = theano.shared(numpy.array([1.] * options['dim_word'], dtype='float32'))
        map_dropout_r = theano.shared(numpy.array([1.] * options['dim_word'], dtype='float32'))
    ...
    map_res = get_layer_constr('ff')(tparams, emb, options,
                                 prefix='ff_map', activ='tanh')

    proj = get_layer_constr(options['encoder'])(tparams, map_res, options,
                                                prefix='encoder',
                                                mask=x_mask,
                                                emb_dropout=map_dropout,
                                                rec_dropout=rec_dropout,
                                                profile=profile)
    ...
    if options['use_dropout']:
        if sampling:
            embr = xr * emb_dropout_r
        else:
            # we drop out the same words in both directions
            embr = xr * emb_dropout[::-1]
    else:
        embr = xr

    map_resr = get_layer_constr('ff')(tparams, embr, options,
                                             prefix='ff_mapr', activ='tanh')

    projr = get_layer_constr(options['encoder'])(tparams, map_resr, options,
                                                 prefix='encoder_r',
                                                 mask=xr_mask,
                                                 emb_dropout=map_dropout_r,
                                                 rec_dropout=rec_dropout_r,
                                                 profile=profile)
...
# build a training model
def build_model_embeddings(tparams, options):
    opt_ret = dict()
    ...
    x, ctx = build_encoder_embeddings(tparams, options, trng, use_noise, x_mask, sampling=False)
    n_samples = x.shape[1]
    n_timesteps_trg = y.shape[0]
...
def build_sampler(tparams, options, use_noise, trng, return_alignment=False):
    ...
    x, ctx = build_encoder_embeddings(tparams, options, trng, use_noise, x_mask=None, sampling=True)
    n_samples = x.shape[2]
...
# build a sampler that receives as input the word embeddings
def build_sampler_embeddings(tparams, options, use_noise, trng, return_alignment=False):
...
def pred_probs(f_log_probs, prepare_data, options, iterator, verbose=True, normalize=False, alignweights=False, embeddings=None):
    probs = []
    n_done = 0
    ...
    for x, y in iterator:
        ...
        x, x_mask, y, y_mask = prepare_data(x, y, embeddings=embeddings,
...
def train(dim_word=100,  # word vector dimensionality
          ...
          dim_map=2000,  # the number of mapping units
          ...
          dropout_map=0.5,  # dropout for the map layer (0: no dropout)
          ...
          embs='/hltsrv7/digangi/external_embeddings/experiment.IWSLT2014/embs.npz',
          emb_type='polyglot'
          ):
    ...
    #embs = numpy.load(embs)['Wemb']
    embeddings = get_embeddings_model(embs, emb_type)
    print("Embedding shape: ", embeddings.shape)
    model_options['dim_external'] = embeddings.shape[1]

    n_words_src = embeddings.shape[0]
    if n_words is None:
    ...
    if use_domain_interpolation:
        print >>sys.stderr, 'Using domain interpolation with initial ratio %s, increase rate %s' % (domain_interpolation_min, domain_interpolation_inc)
    ...
    if valid_datasets and validFreq:
        valid = TextIterator(valid_datasets[0], valid_datasets[1],
                            dictionaries[:-1], dictionaries[-1],
                                      ...
                                      maxlen=maxlen, embeddings=embeddings)
    else:
        valid = None

    print >>sys.stderr, 'Building model'

    params = init_params_embeddings(model_options)
    # reload parameters
    if reload_ and os.path.exists(saveto):
        print >>sys.stderr, 'Reloading model parameters'
    ...
    trng, use_noise, \
        ...
        opt_ret, \
        cost = \
        build_model_embeddings(tparams, model_options)
    ...
    if validFreq or sampleFreq:
        ...
        f_init, f_next = build_sampler_embeddings(tparams, model_options, use_noise, trng)
    ...
    print "Computing the gradients with regards to the following variables:", updated_params.keys()
    ...
    f_grad_shared, f_update = eval(optimizer)(lr, updated_params, grads, inps, cost, profile=profile, hist_gshared=grad_history)
    print >>sys.stderr, 'Done'
    ...
    for eidx in xrange(max_epochs):
        ...
        for x, y in train:
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                print >>sys.stderr, 'Saving the best model...',
                if best_p is not None:
                    params = best_p
                else:
                    params = unzip_from_theano(tparams)
                numpy.savez(saveto, history_errs=history_errs, uidx=uidx, ghist=grad_history, **params)
                json.dump(model_options, open('%s.json' % saveto, 'wb'), indent=2)
                print >>sys.stderr, 'Done'

                # save with uidx
                if not overwrite:
                    print >>sys.stderr, 'Saving the model at iteration {}...'.format(uidx),
                    saveto_uidx = '{}.iter{}.npz'.format(
                        os.path.splitext(saveto)[0], uidx)
                    numpy.savez(saveto_uidx, history_errs=history_errs,
                                uidx=uidx, ghist=grad_history, **unzip_from_theano(tparams))
                    print >>sys.stderr, 'Done'
                    ...
                    for pos in range(x.shape[1]):
                        ...
                        for factor in range(factors):
                            ...
                            if factor+1 < factors:
                                ...
                                sys.stdout.write(' ')
                    #print >>sys.stderr,
                    print >>sys.stderr, 'Truth ', jj, ' : ',
                    for vv in y[:, jj]:
                        if vv == 0:
                            break
                        if vv in worddicts_r[-1]:
                            print >>sys.stderr, worddicts_r[-1][vv],
                        else:
            ...
            if valid and validFreq and numpy.mod(uidx, validFreq) == 0:
                ...
                valid_errs, alignment = pred_probs(f_log_probs, prepare_data,
                                        model_options, valid, embeddings=embeddings)
                ...
                if len(history_errs) > patience and valid_err >= \
                    ...
                    if bad_counter > patience:
                        if use_domain_interpolation and (domain_interpolation_cur < 1.0):
                            ...
                            print >>sys.stderr, 'No progress on the validation set, increasing domain interpolation rate to %s and resuming from best params' % domain_interpolation_cur
                ...
                print >>sys.stderr, 'Valid ', valid_err

                if external_validation_script:
                    print >>sys.stderr, "Calling external validation script"
                    print >>sys.stderr, 'Saving  model...',
                    params = unzip_from_theano(tparams)
                    numpy.savez(saveto +'.dev', history_errs=history_errs, uidx=uidx, ghist=grad_history, **params)
                    json.dump(model_options, open('%s.dev.npz.json' % saveto, 'wb'), indent=2)
                    print >>sys.stderr, 'Done'
                    external_validation_script[-1] = str(uidx)
...
if __name__ == '__main__':
    ...
    training.add_argument('--embs', help='File containing external embeddings, it has no effect'  \
                        'if --use-external-embeddings is not set', metavar='PATH', default='embs.npz')
    training.add_argument('--emb-type', help='File format of the embeddings', default='polygot')
