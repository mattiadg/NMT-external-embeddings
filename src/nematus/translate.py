...
def translate_model(queue, rqueue, pid, models, options, k, normalize, verbose, nbest, return_alignment, suppress_unk):
    ...
    from nmt import (build_sampler, gen_sample, init_params_embeddings)
...
def translate_model_external_embedding(queue, rqueue, pid, models, options, k, normalize, verbose, nbest, return_alignment, suppress_unk):

    from theano_util import (load_params, init_theano_params)
    from nmt import (build_sampler_embeddings, gen_sample, init_params_embeddings)
    ...
    for model, option in zip(models, options):
        ...
        # word index
        f_init, f_next = build_sampler_embeddings(tparams, option, use_noise, trng, return_alignment=return_alignment)
    ...
    def _translate(seq):
        # sample given an input sequence and obtain scores
        sample, score, word_probs, alignment = gen_sample(fs_init, fs_next,
                                                          seq, trng=trng, k=int(k), maxlen=200,
...
def main_embeddings(models, source_file, saveto, save_alignment=None, k=5,
         normalize=False, n_process=5, chr_level=False, verbose=False, nbest=False, suppress_unk=False, a_json=False,
         print_word_probabilities=False):
    ...
    external_embeddings = options[0]['embs']
    embs = get_embeddings_model(external_embeddings, options[0]['emb_type'])
    for option in options:
        option['dim_external'] = embs.shape[1]
    print >>sys.stderr, "Loaded embeddings of shape ", embs.shape
    ...
    for midx in xrange(n_process):
        processes[midx] = Process(
            target=translate_model_external_embedding,
    ...
    def _send_jobs(f):
        ...
        for idx, line in enumerate(f):
            ...
            x = numpy.zeros((len(words), 1, embs.shape[1]), dtype='float32')
            # word2ind = word_dicts[0]
            ...
            x[-1, 0, :] = embs.get('</S>')
            #x = x.reshape(len(words), 1, embs.shape[1])
            queue.put((idx, x))
            source_sentences.append(words)
        return idx + 1, source_sentences
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use-external-embeddings', '-uee', action="store_true", help="Use encoder word embeddings" \
                        "from an external file", dest='use_embeddings')
    parser.add_argument('--external-embeddings', '-ee', help='File containing external embeddings, it has no effect' \
                        'if --use-external-embeddings is not set', dest='embs', default='embs.npz')
    ...
    if args.use_embeddings:
        main_embeddings(args.models, args.input,
